...
def upload_to_facebook(video_path, description):
    ...
    def get_env_secret(name):
        val = os.getenv(name)
        if not val or val == name or val == f"${{{name}}}":
            return None
        return val

    access_token = get_env_secret('FACEBOOK_ACCESS_TOKEN') or get_env_secret('FB_ACCESS_TOKEN')
    page_id = get_env_secret('FACEBOOK_PAGE_ID') or get_env_secret('FB_ID') or get_env_secret('FB_PAGE_ID')
